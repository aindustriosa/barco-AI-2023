...
#define PO_ENABLE_SPEED 100 // 30 se activa el INA
#define PO_TIMER 150
#define PO_STEP 1
...
pinMode(Charge_SC,OUTPUT);
pinMode(SC_EN,OUTPUT);

  digitalWrite(Charge_SC, 0);
  digitalWrite(SC_EN, 0);
}
...
// Control carga SC
    ...
    control_SC = map(rcT[1], 1000, 2000, 0, 100);

  
      if (control_SC < 25) { // menos de 25% de switch Canal 5, abrimos carga de SC y cerramos descarga
        digitalWrite(Charge_SC, 1);
        digitalWrite(SC_EN, 0);
        estado = "Carga";
      }
      if (control_SC > 25 && control_SC < 75) { // más de 25% y menos de 75% de switch Canal 5, cerramos carga de SC y  descarga
        digitalWrite(Charge_SC, 0);
        digitalWrite(SC_EN, 0);
        estado = "Nada";
      }
      if (control_SC > 75) { // más de 75% de switch Canal 5, cerramos carga de SC y abrimos descarga
        digitalWrite(Charge_SC, 0);
        digitalWrite(SC_EN, 1);
        estado = "Descarga";
      }
